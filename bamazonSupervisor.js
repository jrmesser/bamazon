...
var displayTotals = function(input_connection) {
    ...
    input_connection.query(`SELECT department_id AS 'Department ID', departments.department_name AS 'Department Name', over_head_costs AS 'Overhead Costs', COALESCE(SUM(product_sales),0) AS 'Total Sales', SUM(product_sales)-over_head_costs AS 'Total Profit'FROM products JOIN departments ON products.department_name = departments.department_name GROUP BY 'Department ID'`, function (error, results, fields) {
...
var insertNewDepartment = function(input_connection, department_name, department_overhead) {
    input_connection.connect();
    input_connection.query(`INSERT INTO departments (department_name, over_head_costs) VALUES (\'${department_name}\', ${parseFloat(department_overhead)})`, function (error, results, fields) {
//        console.log(results);
